...
#ifndef __GST_DSOSDCOORDRMQ_H__
#define __GST_DSOSDCOORDRMQ_H__
...
#define GST_TYPE_DSOSDCOORDRMQ \
  (gst_ds_osdcoordrmq_get_type())
#define GST_DSOSDCOORDRMQ(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj),GST_TYPE_DSOSDCOORDRMQ,GstDsOsdCoordRmq))
#define GST_DSOSDCOORDRMQ_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST((klass),GST_TYPE_DSOSDCOORDRMQ,GstDsOsdCoordRmqClass))
#define GST_IS_DSOSDCOORDRMQ(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj),GST_TYPE_DSOSDCOORDRMQ))
#define GST_IS_DSOSDCOORDRMQ_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_TYPE((klass),GST_TYPE_DSOSDCOORDRMQ))
/* Version number of package */
#define VERSION "1.8.2"
#define PACKAGE_DESCRIPTION "Gstreamer plugin to draw rectangles, text and coordinate"
...
typedef struct _GstDsOsdCoordRmq GstDsOsdCoordRmq;
typedef struct _GstDsOsdCoordRmqClass GstDsOsdCoordRmqClass;

/**
 * GstDsOsdCoordRmq element structure.
 */
struct _GstDsOsdCoordRmq
{
  ...
  /** Pointer to the dsosdcoordrmq context. */
  void *dsosdcoordrmq_context;
  /** Enum indicating how the objects are drawn,
      i.e., CPU, GPU or VIC (for Jetson only). */
  NvOSD_Mode dsosdcoordrmq_mode;
  ...
  gboolean draw_mask;
  /** Boolean indicating whether coordinate is to be displayed. */
  gboolean display_coord;
